...
public class GameManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("Cannot load level data: Data/LevelGenerator/Level/Level " + currlevel);
            return;
        }

        ValidateLevelData(currLevelData);

        GameObject boardPrefab = Resources.Load<GameObject>("Prefab/Board"+currLevelData.rows.ToString()+"Cell");
        if (boardPrefab == null)
        {
            Debug.LogError("Cannot load board prefab: Prefab/Board" + currLevelData.rows + "Cell");
            return;
        }
    ...
    {
        int nonZeroCount = 0;
        int answerCellCount = 0;

        for (int i = 0; i < levelData.rows; i++)
        {
            for (int j = 0; j < levelData.columns; j++)
            {
                if (levelData.board[i].column[j] != 0)
                {
                    nonZeroCount++;
                }

                if (levelData.board[i].column2[j])
                {
                    answerCellCount++;
                }
            }
        ...
        if (nonZeroCount == 0)
        {
            Debug.LogWarning("Level " + currlevel + " has all cell values = 0. Check level asset data.");
        }

        if (answerCellCount == 0)
        {
            Debug.LogWarning("Level " + currlevel + " has no answer cells selected (column2 all false).");
        }
    }
